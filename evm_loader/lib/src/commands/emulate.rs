...
use evm_loader::allocator::acc_allocator;
...
async fn emulate_trx_single_step<T: Tracer>(
    ...
    let (exit_status, steps_executed, step_on_solana, tracer, timestamped_contracts) = {
        ...
        let mut evm = match Machine::new(tx, origin, acc_allocator(), &mut backend, tracer).await {
            Ok(evm) => evm,
            Err(e) => {
    ...
    let (exit_status, steps_executed, step_on_solana, tracer, timestamped_contracts) = {
        ...
        for execution_step in &execution_map.steps {
            if execution_step.is_reset {
                ...
                evm = match Machine::new(&tx, origin, acc_allocator(), &mut backend, tracer_result)
                    .await
                {
                    Ok(evm) => evm,
                    Err(e) => {
