use ethnum::U256;
use evm_loader::account::{Holder, StateAccount, TAG_HOLDER, TAG_STATE, TAG_STATE_FINALIZED};
use serde::Serialize;
...
pub fn read_holder(program_id: &Pubkey, info: AccountInfo) -> NeonResult<GetHolderResponse> {
    ...
    let tag = evm_loader::account::tag(program_id, &info)?;
    match tag {
        TAG_HOLDER => {
        ...
        TAG_STATE_FINALIZED => {
            evm_loader::account::set_tag(program_id, &info, TAG_STATE)?;
            read_holder(program_id, info)
        }
        TAG_STATE => {
