...
pub fn use_original_stubs_for_thread(new: bool) -> bool {
    USE_ORIGINAL_STUBS
        .with(|invoke_context| invoke_context.replace(new))
}
...
impl EmulatorStubs {
    pub async fn new(rpc: &impl Rpc, original_stubs: Box<dyn SyscallStubs>) -> Result<Box<EmulatorStubs>, NeonError> {
        ...
        Ok(Box::new(Self { rent, original_stubs }))
    }
}
...
impl SyscallStubs for EmulatorStubs {
    ...
    fn sol_invoke_signed(
            ...
            if is_original_stubs_for_thread() {
                self.original_stubs.sol_invoke_signed(_instruction, _account_infos, _signers_seeds)
            } else {
