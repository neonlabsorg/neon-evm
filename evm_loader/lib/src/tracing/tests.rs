...
use crate::account_storage::{make_solana_program_address, EmulatorAccountStorage};
...
use evm_loader::evm::{ExitStatus, Machine};
...
async fn test_trace_contract_creation() {
    ...
    let (_code, emulation_result) = trace_contract_creation(gas_used, &tracer).await;
...
async fn trace_contract_creation(
    ...
    let test_account_storage = test_emulator_account_storage(program_id, &rpc).await;
...
fn account_with_data(program_id: &Pubkey, address: Address, data: Data) -> (Pubkey, Account) {
    let mut account = Account::new(0, 10000, program_id);
    let (pubkey, _) = make_solana_program_address(&address, program_id);

    EthereumAccount::init(
        program_id,
        &writable_account_info(&pubkey, &mut account),
        data,
    )
    .unwrap();
...
async fn test_trace_increment_call() {
    ...
    let rpc = increment_call_test_rpc(program_id, origin, target, index).await;
...
async fn increment_tx_params(
    ...
    origin: Address,
    target: Address,
    test_account_storage: &impl AccountStorage,
) -> Transaction {
...
async fn increment_call_test_rpc(
    ...
    let (target_pubkey, _) = make_solana_program_address(&target, &program_id);
    let mut target_account = Account::new(0, 10000, &program_id);
    {
        ...
        let ethereum_account = EthereumAccount::init(
            &program_id,
            &account_info,
            Data {
                address: target,
        ...
        ethereum_account
            ...
            .unwrap()
            .code()
            .copy_from_slice(&code_vec);

        let index: usize = index.as_usize() * 32;
        ethereum_account.contract_data().unwrap().storage()[index..index + 32]
            .copy_from_slice(&U256::from_str_hex("0x0f").unwrap().to_be_bytes());
    }
...
fn set_up_rent_sysvar() -> Rent {
    ...
    solana_sdk::program_stubs::set_syscall_stubs(Box::new(Stubs::new_rent(rent)));
    rent
}
...
async fn test_trace_transfer_transaction() {
    ...
    let rpc = transfer_transaction_rpc(program_id, origin, target);
    ...
    assert_eq!(
        backend.balance(&target).await.unwrap()
            - test_account_storage.balance(&target).await.unwrap(),
        value
    );
}
...
fn transfer_transaction_rpc(program_id: Pubkey, origin: Address, target: Address) -> impl Rpc {
    let rent = set_up_rent_sysvar();
...
async fn emulate_trx<B: AccountStorage>(
    ...
    EmulationResult {
        ...
        used_gas: 0,
        actions,
        states: build_states(backend, origin, tx_fee).await.unwrap(),
    }
}
