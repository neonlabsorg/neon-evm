...
pub async fn build_state_diff(
    ...
    for address in backend.backend.all_addresses().iter() {
        state_diff.insert(
            H160::from(address.as_bytes()),
            AccountDiff {
                balance: build_balance_diff(from, tx_fee, backend, address).await?,
                nonce: build_nonce_diff(backend, address).await?,
                code: build_code_diff(backend, address).await?,
...
async fn build_nonce_diff(
    ...
    Ok(diff_new_u256(
        backend.backend.nonce(address).await.into(),
