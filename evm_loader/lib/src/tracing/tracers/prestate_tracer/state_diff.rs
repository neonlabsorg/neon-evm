...
pub async fn build_states(
    ...
    Ok(PrestateTracerDiffResult {
        post: build_post_state(from, tx_fee, backend).await?,
        pre: build_pre_state(backend).await?,
    })
}
...
async fn build_pre_state(
    ...
    for address in backend.backend.all_addresses().into_iter() {
        pre_state.insert(
            address,
            Account {
                balance: backend.backend.balance(&address).await.map(to_web3_u256),
                nonce: backend.backend.nonce(&address).await,
...
async fn build_post_state(
    ...
    for address in backend.backend.all_addresses().into_iter() {
        let mut balance = backend.balance(&address).await?;
        ...
        post_state.insert(
            ...
            Account {
                ...
                nonce: Some(backend.nonce(&address).await?),
