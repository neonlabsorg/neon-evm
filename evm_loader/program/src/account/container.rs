...
impl<T: AccountRead> Reference<T> {
    ...
    #[must_use]
    pub fn into_raw_account(self) -> T {
        self.account
    }
}
...
impl<T: Account> Container<T> {
    ...
    pub fn unwrap_container(self) -> Result<impl Account> {
    ...
    /// # Safety
    /// Invalidates all indexes. Should not be used in normal transaction processing.
    pub unsafe fn remove_account(&mut self, reference: Reference<impl Account>) -> Result<()> {
