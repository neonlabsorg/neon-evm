...
fn expand_header<'a, From: AccountHeader, To: AccountHeader>(
    ...
    unsafe {
        ...
        let ptr = account.try_borrow_mut_data()?.as_mut_ptr();
        let header_ptr = ptr.add(ACCOUNT_PREFIX_LEN);
        let src = header_ptr.add(from_len);
        let dst = header_ptr.add(to_len);
        sol_memmove(dst, src, data_len);

        src.write_bytes(0, to_len - from_len);

        let version_ptr = ptr.add(HEADER_VERSION_OFFSET);
        version_ptr.write(To::VERSION);
    }
