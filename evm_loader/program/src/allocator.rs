...
#[allow(clippy::cast_possible_truncation)] // HEAP_START_ADDRESS < usize::max
const HOLDER_HEAP_START_ADDRESS: usize = 0x400000000u64 as usize + FIRST_ACCOUNT_DATA_OFFSET;
const_assert_eq!(HOLDER_HEAP_START_ADDRESS % align_of::<Heap>(), 0);

const HOLDER_HEAP_OBJ_ADDRESS: usize = HOLDER_HEAP_START_ADDRESS;

const HOLDER_STATE_OBJ_ADDRESS: usize = HOLDER_HEAP_OBJ_ADDRESS + size_of::<Heap>();
const_assert_eq!(HOLDER_STATE_OBJ_ADDRESS % align_of::<PersistentState>(), 0);

const HOLDER_HEAP_DATA_ADDRESS: usize = HOLDER_STATE_OBJ_ADDRESS + size_of::<PersistentState>();

const HOLDER_HEAP_SIZE: usize = 10 * 1024;

pub fn state<'a, 'r>() -> *mut PersistentState<'a, 'r> {
    let state_ptr: *mut PersistentState = HOLDER_STATE_OBJ_ADDRESS as *mut PersistentState;
    state_ptr
}
...
fn holder_account_heap() -> &'static mut Heap {
    ...
    if heap.bottom().is_null() {
        let start = HOLDER_HEAP_DATA_ADDRESS as *mut u8;
        //TODO proper calculations of heap length, taking into account the real account data length and its resizes.
        let size = HOLDER_HEAP_SIZE - (HOLDER_HEAP_DATA_ADDRESS - HOLDER_HEAP_START_ADDRESS);
        unsafe { heap.init(start, size) };
    }
