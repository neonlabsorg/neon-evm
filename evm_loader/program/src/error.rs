...
pub enum Error {
    ...
    AccountNotWritable(Pubkey), // TODO missing check on ether_balance, ether_contract, ether_storage

    #[error("Account {0} - not signer")]
    AccountNotSigner(Pubkey),

    #[error("Account {0} - not rent exempt")]
    AccountNotRentExempt(Pubkey), // TODO missing check on ether_balance, ether_contract, ether_storage - never used

    #[error("Account {0} - already initialized")]
    AccountAlreadyInitialized(Pubkey), // TODO missing check on ether_balance, ether_contract, ether_storage - never used
