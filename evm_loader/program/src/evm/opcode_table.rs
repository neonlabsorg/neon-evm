...
impl<'a, B: Database + 'a> Machine<B> {
    pub const OPCODES: [fn(
        &'a mut Self,
        &mut B,
    ) -> Pin<Box<dyn Future<Output = Result<Action>> + 'a>>; 256] = {
        let opcodes: [fn(
            &'a mut Self,
            &mut B,
        ) -> Pin<Box<dyn Future<Output = Result<Action>> + 'a>>; 256] = [Self::opcode_unknown; 256];
