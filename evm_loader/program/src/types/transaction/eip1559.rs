...
impl<T: AsRef<[u8]>> Transaction for Eip1559<T> {
...
pub trait PriorityFeeTransaction: Transaction {
    fn max_fee_per_gas(&self) -> U256;
    fn max_priority_fee_per_gas(&self) -> U256;
}

impl<T: AsRef<[u8]>> PriorityFeeTransaction for Eip1559<T> {
    fn max_fee_per_gas(&self) -> U256 {
        self.max_fee_per_gas
    }
...
fn calculate_signed_hash(signed_data: &[u8]) -> keccak::Hash {
    ...
    keccak::hashv(&[&[0x02], &header, signed_data])
}
